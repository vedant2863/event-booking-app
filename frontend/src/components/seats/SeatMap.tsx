...
export const SeatMap = ({ layout, onSelectionChange, maxSelect = 8, realtimeUpdates = [] }: Props) => {
      ...
      for (const update of realtimeUpdates) {
        for (const section of Object.values(updated)) {
          for (const row of Object.values(section as any)) {
            ...
            if (seat) (seat as any).status = update.status === 'seat:locked' ? 'locked' : update.status === 'seat:booked' ? 'booked' : 'available';
          }
        }
      }
      return updated;
