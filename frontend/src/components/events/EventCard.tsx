...
export const EventCard = ({ event }: Props) => {
  ...
  return (
    <Link to={`/events/${event._id}`} className="card group hover:border-brand-500/50 transition-all duration-300 hover:-translate-y-1">
      {/* Banner */}
      <div className="relative h-48 bg-gradient-to-br from-brand-900 to-gray-800 overflow-hidden">
        ...
        <span className={clsx('badge absolute top-3 left-3', CATEGORY_COLORS[event.category] || CATEGORY_COLORS.other)}>
          {event.category}
        </span>
      </div>
          ...
          <div className="text-right">
            ...
            <div className="w-24 h-1.5 bg-gray-800 rounded-full mt-1">
              <div
                ...
                style={{ width: `${Math.max(5, (event.availableSeats / event.totalSeats) * 100)}%` }}
              />
            </div>
          </div>
        </div>
