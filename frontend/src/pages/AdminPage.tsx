  ...
  if (loading) return (
    <div className="max-w-6xl mx-auto px-4 py-10 animate-pulse space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {Array.from({length:4}).map((_,i) => <div key={i} className="card h-28 bg-gray-800" />)}
      </div>
    </div>
  );
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'Published Events', value: stats.totalEvents, icon: Calendar, color: 'text-purple-400', bg: 'bg-purple-500/10' },
            { label: 'Confirmed Bookings', value: stats.totalBookings, icon: Ticket, color: 'text-green-400', bg: 'bg-green-500/10' },
            { label: 'Total Revenue', value: `₹${stats.totalRevenue.toLocaleString()}`, icon: TrendingUp, color: 'text-brand-400', bg: 'bg-brand-500/10' },
        ...
        <div className="card divide-y divide-gray-800">
          ...
          {stats.recentBookings.slice(0, 8).map((booking: any) => (
            <div key={booking._id} className="p-4 flex items-center justify-between text-sm">
              <div>
                <p className="text-white font-medium">{booking.eventId?.title || 'Event'}</p>
                <p className="text-gray-400">{booking.userId?.username} · {booking.bookingReference}</p>
              </div>
          ...
          {bookings.map((booking: any) => (
            <div key={booking._id} className="p-4 flex items-center justify-between text-sm">
              <div>
                ...
                <p className="text-gray-400">{booking.userId?.username} · {booking.eventId?.title}</p>
              </div>
              <div className="text-right space-y-0.5">
                <p className="text-white">₹{booking.totalAmount?.toLocaleString()}</p>
                <span className={`badge text-xs ${
                  booking.bookingStatus === 'confirmed' ? 'bg-green-500/10 text-green-400' :
                  booking.bookingStatus === 'cancelled' ? 'bg-red-500/10 text-red-400' :
